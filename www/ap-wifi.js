(function () {
  ...
  var I = {
    zh: {
      ...
      secured: "需要密码",
      open: "开放网络",
      apSelected: "已选择 {ssid}，请输入密码后连接。",
      apScanningHint: "正在扫描附近 WiFi...",
      scanning: "正在扫描...",
      apScanDone: "扫描完成，请选择网络或手动输入 SSID。",
      apScanFailHint: "扫描失败也可以继续配网，请手动输入 SSID。",
      apScanFail: "扫描失败：请手动输入 SSID 后连接。",
      apSsidRequired: "请先输入 WiFi 名称 SSID。",
      apConnecting: "正在连接 {ssid}，设备可能会在成功后重启...",
      apConnectSent: "连接请求已发送。若连接成功，设备会保存设置并重启。",
      apConnectFail: "连接失败，请检查 SSID 和密码。"
    },
    en: {
      title: "Connect WiFi",
      ...
      open: "open network",
      apSelected: "Selected {ssid}. Enter the password, then connect.",
      apScanningHint: "Scanning nearby WiFi...",
      scanning: "Scanning...",
      apScanDone: "Scan complete. Select a network or enter an SSID manually.",
      apScanFailHint: "Scanning failed. You can still enter an SSID manually.",
      apScanFail: "Scan failed. Enter an SSID manually, then connect.",
      apSsidRequired: "Enter the WiFi SSID first.",
      apConnecting: "Connecting to {ssid}. The device may restart after success...",
      apConnectSent: "Connection request sent. If successful, the device saves settings and restarts.",
      apConnectFail: "Connection failed. Check the SSID and password."
    }
  };

  function text(key, args) {
    var zh = I.zh || {};
    var value = (I[lang] && I[lang][key]) || zh[key] || key;
  ...
  function applyLang() {
    ...
    scanHint.textContent = text(
      statusKey === "apScanFail" ? "apScanFailHint" : "scanHint",
    );
  ...
  function networkLabel(item) {
    ...
    var signal = Number.isFinite(strength)
      ? strength + " dBm"
      : text("unknownSignal");
  ...
  function renderNetworks(networks) {
    ...
    networksCache.forEach(function (item) {
      ...
      row.onclick = function () {
        ...
        passwordInput.focus();
        setStatus("apSelected", "", { ssid: item.ssid });
      };
  ...
  async function scanNetworks() {
    ...
    scanHint.textContent = text("apScanningHint");
    setStatus("scanning");
    try {
      ...
      setStatus("apScanDone", "ok");
    } catch (error) {
      renderNetworks([]);
      scanHint.textContent = text("apScanFailHint");
      setStatus("apScanFail", "error");
    } finally {
  ...
  async function connectWifi(event) {
    ...
    if (!ssid) {
      ssidInput.focus();
      setStatus("apSsidRequired", "error");
      return;
    }
    connectBtn.disabled = true;
    setStatus("apConnecting", "", { ssid: ssid });
    try {
      var body = new URLSearchParams();
      body.set("ssid", ssid);
      body.set("password", passwordInput.value);
      body.set("persistent", "true");
      var response = await fetch("/connect", {
        method: "POST",
        headers: { "Content-Type": "application/x-www-form-urlencoded" },
        body: body.toString(),
      });
      var responseText = await response.text();
      if (!response.ok) throw new Error(responseText || text("apConnectFail"));
      setStatus("apConnectSent", "ok");
    } catch (error) {
      statusEl.textContent = error.message || text("apConnectFail");
      statusEl.className = "status error";
      statusKey = "apConnectFail";
      statusType = "error";
      statusArgs = [];
